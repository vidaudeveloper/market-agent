...
图表数值必须与报告正文、Project Facts 一致：

- 图表的 `data` 只能来自报告已引用、且能回溯到 `facts[].evidence_ids` 的数字。
- 禁止为了图表好看临时编造或凑整与正文不一致的数值。
- 数值为估算时，图表标题或图注标注「估算」，与正文口径一致。
- 事实包契约见 `skills/PROJECT-FACTS.md`。
