...
2. `skills/DATA-SOURCE.md`
3. 参考案例：`templates/reference/NARRAK-Electric-Bike-&-Yatop-TTS合作方案.md`、`绿巨能LLano-&-Yatop-TTS全案运营合作方案.md`
4. 填空模板：`templates/tts-partnership.template.md`
5. 机构默认配置：`templates/config/agency-defaults.json`（**署名 Vidau，禁止 Yatop/雅拓**）
...
```
1. 读 intake + agency-defaults
2. WebFetch Amazon 产品页；有 TTS 链接则抓取店铺信息
3. chuhaijiang-data：搜竞品店铺/类目（见 chuhaijiang skill 字段清单）
4. 复制 tts-partnership.template.md → output/{BRAND}-TTS合作方案-{DATE}.md
5. 按下方「强制章节」填满；竞品每家写「启示」≥3 条
6. 插入 ≥2 处 <!-- chart --> 标记
7. 自检 DELIVERY-STANDARD 清单
8. node scripts/feishu-export.js output/xxx.md "标题" --charts
```
...
3. **店铺&产品分析**（四段：店铺/产品/总结/TK阶段）
4. **竞品&市场**（≥2 竞品，每家：达人/内容/渠道 + **启示**）
