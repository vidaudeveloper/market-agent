...
2. `skills/chuhaijiang-data/SKILL.md`（对标数据 + 截图）
3. `skills/tts-pricing-logic/SKILL.md`（有目标 GMV 时联动报价）
...
4. 写「相对位置」必须含倍数差距；附出海匠截图章节
5. 问题诊断：≥1 致命 + ≥2 严重，带 ⚠️
6. 分阶段 GMV 表 + 财务假设 + chart 标记
7. 有 GMV 目标 → 引用 tts-pricing-logic 输出人手/月费测算
